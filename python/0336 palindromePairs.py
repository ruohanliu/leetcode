class Solution:
    def palindromePairs(self, words: List[str]) -> List[List[int]]:
        """
            #palindrome #hard #important

            related: 214 Shortest Palindrome
        """
        ...
        for j,word in enumerate(words):
            ...
            for i in range(len(word)+1):
                if word.startswith(reverse[i:]):
                    k = wordDict.get(reverse[:i],j)
                    if k != j:
                        ans.add((k,j))
                if reverse.startswith(word[i:]):
                    k = wordDict.get(word[:i][::-1],j)
                    if k != j:
                        ans.add((j,k))
        return ans
